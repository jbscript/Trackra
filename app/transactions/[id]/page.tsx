...
function getCategoryIcon(categoryName: string) {
  ...
  if (name.includes("transport") || name.includes("gas") || name.includes("cab")) return Car
  if (name.includes("hous") || name.includes("rent") || name.includes("mortgage")) return Home
  if (name.includes("utilit") || name.includes("bill") || name.includes("water") || name.includes("electric")) return Zap
  if (name.includes("health") || name.includes("medical") || name.includes("doctor")) return Activity
  if (name.includes("gift") || name.includes("family")) return Gift
  if (name.includes("invest") || name.includes("transfer")) return ArrowRightLeft
    ...
    <div className="min-h-screen bg-[#0e0e0e] text-white p-4 md:p-8 font-manrope selection:bg-[#5cfd80]/30 selection:text-[#5cfd80]">
      <div className="mx-auto max-w-xl pb-24">
        ...
        <div className="flex items-center justify-between mb-16">
          <Link href="/transactions" className="flex items-center gap-1 text-[#5cfd80] hover:text-[#5cfd80]/80 transition-colors">
            <ChevronLeft size={24} />
            <span className="font-bold tracking-wide">Details</span>
          </Link>
          <button className="text-[#5cfd80] hover:text-[#5cfd80]/80 font-bold tracking-wide transition-colors">
            Edit
          </button>
        </div>
        ...
        <div className="space-y-4 mb-8">
          <div className="bg-[#161618] rounded-[24px] p-6 border border-white/5 shadow-[0_4px_20px_rgba(0,0,0,0.5)] flex flex-col justify-center">
            <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">Date & Time</h3>
            <p className="font-bold tracking-wide text-gray-100 text-[15px]">
              {transaction.transactionDate.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })} &middot; {transaction.transactionDate.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-[#161618] rounded-[20px] p-6 border border-white/5 shadow-[0_4px_20px_rgba(0,0,0,0.5)] flex flex-col justify-center">
              <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">Category</h3>
              <div className="flex items-center gap-3">
                <div className="w-2.5 h-2.5 rounded-full bg-[#5cfd80] shadow-[0_0_8px_rgba(92,253,128,0.5)]" />
                <p className="font-bold tracking-wide text-gray-100 text-[15px]">{transaction.category.name}</p>
              </div>
            </div>
            
            <div className="bg-[#161618] rounded-[20px] p-6 border border-white/5 shadow-[0_4px_20px_rgba(0,0,0,0.5)] flex flex-col justify-center">
              <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">Source</h3>
              <p className="font-bold tracking-wide text-gray-100 text-[15px]">{transaction.account.name}</p>
            </div>
          </div>
          ...
          <div className="bg-[#161618] rounded-[24px] border border-white/5 shadow-[0_4px_20px_rgba(0,0,0,0.5)] overflow-hidden h-[180px] relative">
            <div className="absolute inset-0 opacity-15 overflow-hidden flex items-center justify-center">
              <svg width="400" height="400" xmlns="http://www.w3.org/2000/svg" stroke="#ffffff" strokeWidth="0.5" fill="none">
            ...
            <div className="absolute bottom-5 left-5 flex items-center gap-2 bg-[#121214]/80 backdrop-blur-sm px-4 py-2 rounded-xl border border-white/10">
              ...
              <span className="text-xs font-bold tracking-wide text-white">452 Park Ave, New York</span>
            </div>
          </div>
          ...
          <div className="bg-[#161618] rounded-[24px] p-6 pt-10 border border-white/5 shadow-[0_4px_20px_rgba(0,0,0,0.5)] flex flex-col justify-center relative">
            <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#122217] border border-[#5cfd80]/30 shadow-[0_4px_10px_rgba(0,0,0,0.3)] rounded-full px-4 py-1.5 flex items-center gap-2">
              <div className="w-2.5 h-2.5 rounded-full bg-[#5cfd80] shadow-[0_0_8px_rgba(92,253,128,0.5)]" />
              <span className="text-[10px] font-extrabold text-[#5cfd80] tracking-widest uppercase">Cleared by Bank</span>
            </div>
            
            <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">Notes</h3>
            <p className="font-medium tracking-wide text-gray-300 italic text-[15px] leading-relaxed">
              "{transaction.note || `Standard ${transaction.category.name} transaction.`}"
            </p>
          </div>
        </div>
